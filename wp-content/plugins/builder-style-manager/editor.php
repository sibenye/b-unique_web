...
Change Notes:
	1.0.0 - 2011-05-16 - Chris Jean
		Updated _default_menu_function to add_theme_page
		Added _delete_stylesheet function to streamline calls
		Added _delete_old_stylesheets function
		Fixed ability to rename a style to the name of another style
		Added border options to Basic Module Styling and Widget Bar Module
	1.0.1 - 2011-07-06 - Chris Jean
		Updated the structure to support the new Builder 3.0 classes
	1.1.0 - 2011-08-18 - Chris Jean
		Added Header Module support
	1.1.1 - 2011-08-19 - Chris Jean
		Removed duplicated options from Post/Page Content Styling > Page Title
	1.1.2 - 2013-01-10 - Chris Jean
		Misc file cleanup.
	1.2.0 - 2013-01-21 - Chris Jean
		Changed var to point to the new data.
		Updated to use ITDialog.
		Fixed styling issues.
		Removed margin, padding, and many border options.
		Added Navigation Module active link styling.
		Improved Module background styling selectors.
	1.3.0 - 2013-01-22 - Chris Jean
		Removed clear_fade_messages.
		Added format notice.
		Improved comment selector.
		Re-added specific border options.
	1.4.0 - 2013-02-18 - Chris Jean
...
class BuilderStyleManager extends ITCoreClass {
	...
	function init() {
		...
		add_filter( "get_user_option_closedpostboxes_{$this->_var}", array( &$this, 'filter_closedpostboxes' ), 10, 3 );
	}
...
?>
	<script type="text/javascript">
		...
		<?php if ( empty( $this->_options['styles'] ) ) : ?>
			...
			win.jQuery(".wrap #styles:first").replaceWith("<br /><p>No styles have been created. Please click the Create Style button to create a new style.</p>");
		<?php else : ?>
...
?>
	<?php $form->start_form(); ?>
		...
		<div style="text-align:center;">
			...
			<?php $form->add_submit( 'cancel', array( 'value' => 'Cancel', 'class' => 'button-secondary', 'onclick' => 'it_dialog_remove(); return false;' ) ); ?>
		</div>
		
		<?php $form->add_hidden( 'delete_style_screen', $style ); ?>
		<?php $form->add_hidden( 'render_clean', $_REQUEST['render_clean'] ); ?>
	<?php $form->end_form(); ?>
<?php
...
<?php
	...
	function _show_other_styles_exist_notice() {
		...
		$storage =& new ITStorage( 'builder_style_manager', true );
		...
		echo '<p>Since you are running a responsive-ready child theme, the listing below will only show the Styles created in this format. Any Styles in the other format still exist and can still be used if you switch back to a non-responsive child theme.</p>';
...
?>
	<div class="wrap">
		<?php $form->start_form(); ?>
			<div id="icon-ithemes-builder-theme" class="icon32">
				<br />
			</div>
			...
			<?php if ( ! empty( $styles ) ) : ?>
				...
				<br class="clear" />
			<?php else : ?>
				<p>No styles have been created. Please click the Create Style button to create a new style.</p>
			<?php endif; ?>
	...
	<tr <?php if ( false === $display ) echo 'style="display:none;" '; ?>id="entry-<?php echo $style_id; ?>"<?php echo $class; ?>>
		<td>
			...
			<div class="row-actions">
				...
				<span class="duplicate"><a href="<?php echo ITDialog::get_link( "{$this->_self_link}&duplicate_style_screen={$style_id}" ); ?>" class="it-dialog" title="Duplicate Style">Duplicate</a> | </span>
				<span class="delete"><a href="<?php echo ITDialog::get_link( "{$this->_self_link}&delete_style_screen={$style_id}" ); ?>" class="it-dialog" title="Delete Style">Delete</a></span>
			</div>
		</td>
...
//			echo '<div class="row-actions"><a href="' . ITDialog::get_link( "{$this->_self_link}&set_global_style_screen={$style_id}" ) . '" class="it-dialog" title="Set this style as global">Set as global</a></div>';
		}
	}
	...
	<div class="wrap">
		...
		<?php $form->start_form(); ?>
			...
			<?php
				...
				$form->add_hidden( 'normalize-styling-css-href', "{$this->_plugin_url}/css/normalize-styling.css", true );
...
<?php
	...
	function _add_meta_box( $title, $position, $closed = true ) {
		...
		$this_var = preg_replace( '/_2$/', '', $this->_var );
		
		add_meta_box( "$this_var-style-$var", $title, array( &$this, "meta_box_style_$method_var" ), $this->_var, $position );
		
		if ( true === $closed )
			$this->_closed_meta_boxes[] = "$this_var-style-$var";
	}
	...
	function _print_meta_boxes() {
		...
		$form->add_hidden( 'module_bg-selector', '.builder-module', true );
...
/*		$form->add_hidden( 'module_outside_gap_top-selector', '.builder-module .builder-module-block', true );
		...
		$form->add_hidden( 'widget_bar_modules_bg-selector', '.builder-module.builder-module-widget-bar', true );
...
//		$form->add_hidden( 'widget_bar_widgets_vertical_gap-selector', '.builder-module-widget-bar .widget', true );
		...
		$form->add_hidden( 'navigation_bg-selector', '.builder-module-background-wrapper.builder-module-navigation-background-wrapper,.builder-module.builder-module-navigation,.builder-module-navigation li a', true );
		$form->add_hidden( 'navigation_bg_hover-selector', '.builder-module-navigation li a:hover', true );
		$form->add_hidden( 'navigation_bg_active-selector', '.builder-module-navigation .current_page_item a,.builder-module-navigation .current-cat a,.builder-module-navigation .current-menu-item a', true );
		...
		$form->add_hidden( 'navigation_text_active-selector', '.builder-module-navigation .current_page_item a,.builder-module-navigation .current-cat a,.builder-module-navigation .current-menu-item a', true );
		...
		$form->add_hidden( 'comments_comment-selector', '#comments li.comment', true );
...
<?php
	...
	function meta_box_style_site_font() {
		...
		$this->_add_section_description( 'These options modify the base font on the site. These settings can be overridden for specific areas by using the options offered in other sections.' );
...
<?php
	...
	function meta_box_style_container() {
		...
		$this->_add_section_description( 'These options style the container. The container is the wrapper div that contains all the modules.' );
		
		$this->_add_background_options( $group );
		$this->_add_border_options( $group );
	}
	...
	function meta_box_style_basic_module_styling() {
		...
		$this->_add_section_description( 'These options set some basic module styling options that apply to all modules. The background options can be overridden for specific modules in each modules\' options.' );
		
		$this->_add_background_options( "{$group}_bg" );
		$this->_add_border_options( "{$group}_bg" );
	}
...
<?php
	...
	function meta_box_style_post_page_content_styling() {
		...
		$this->_add_section_description( 'These options style the page and post content.' );
		
		$this->_add_basic_text_options( $group );
		$this->_add_basic_link_options( "{$group}_link" );
		$this->_add_title_options( 'post_title', 'Post Title', '', true );
		$this->_add_title_options( 'page_title', 'Page Title', '', true );
	}
	
	function meta_box_style_comments_styling() {
		$form = $this->_form;
		
		$this->_add_section_title( 'Comments Section' );
		$this->_add_section_description( 'Styling for the wrapper around all the comments' );
		
		$group = 'comments';
		
		$this->_add_basic_heading_options( "{$group}_heading" );
		
		$this->_start_show_hide_section( 'Spacing' );
...
<?php
		...
		$this->_end_show_hide_section();
		
		$this->_add_background_options( $group );
		$this->_add_border_options( $group, 'Borders', false, true );
		
		
		$this->_add_section_title( 'Individual Comments' );
		$this->_add_section_description( 'These styles are added to each comment.' );
		
		$group = 'comments_comment';
	...
	function meta_box_style_navigation_module() {
		$this->_add_section_title( 'Basic Styling' );
		$this->_add_section_description( 'These settings style the unhovered appearance of the Navigation Modules.' );
		
		$this->_add_basic_text_options( 'navigation_text' );
		$this->_add_background_options( 'navigation_bg' );
		$this->_add_border_options( 'navigation_border' );
		
		
		$this->_add_section_title( 'Hover Styling' );
		$this->_add_section_description( 'These settings style the appearance of the Navigation Modules\' links when hovered over.' );
		
		$this->_add_basic_text_options( 'navigation_text_hover' );
		$this->_add_background_options( 'navigation_bg_hover' );
		
		
		$this->_add_section_title( 'Active Styling' );
		$this->_add_section_description( 'These settings style the appearance of the Navigation Modules\' active link.' );
		
		$this->_add_basic_text_options( 'navigation_text_active' );
		$this->_add_background_options( 'navigation_bg_active' );
	}
	...
	function meta_box_style_module_sidebars() {
		...
		$this->_add_section_title( 'Sidebars' );
		$this->_add_section_description( 'These options style the background behind the modules in all module sidebars. Note that Widget Bar module sidebars are styled separately in the Widget Bar Modules section.' );
		
		$this->_add_background_options( $group );
		
		
		$group = 'module_sidebar_widgets';
		
		$this->_add_section_title( 'Widgets' );
		$this->_add_section_description( 'These options style the widgets in all module sidebars. Note that Widget Bar module widgets are styled separately in the Widget Bar Modules section.' );
		...
		$this->_add_title_options( "{$group}_title", 'Widget Titles', 'These options style each widget\'s title.' );
	}
	...
	function meta_box_style_widget_bar_module() {
		...
		$this->_add_section_title( 'Module Wrapper' );
		$this->_add_section_description( 'These options style the Widget Bar module container that holds all the widget areas.' );
		
		$this->_add_background_options( "{$group}_bg" );
		$this->_add_border_options( "{$group}_bg" );
		
		
		$group = 'widget_bar_widgets';
		
		$this->_add_section_title( 'Widgets' );
		$this->_add_section_description( 'These options style the widgets in all Widget Bar modules. Note that widgets for other modules are styled separately in the Module Sidebars section.' );
		...
		$this->_add_title_options( "{$group}_title", 'Widget Titles', 'Thse options style each widget\'s title.' );
	}
...
<?php
		...
		echo ' ';
		$form->add_submit( 'cancel', array( 'value' => 'Cancel', 'class' => 'button-secondary', 'onclick' => 'it_dialog_remove(); return false;' ) );
		
		echo "</div>\n";
		
		$form->add_hidden( 'upload_background_image_save', '1' );
		$form->add_hidden( 'upload_background_image', $_REQUEST['upload_background_image'] );
		$form->add_hidden( 'render_clean', $_REQUEST['render_clean'] );
...
?>
		<a href="<?php echo ITDialog::get_link( "{$this->_self_link}&upload_background_image={$selector}-background-image" ); ?>" class="it-dialog <?php echo $selector; ?>-background-image-upload-link" style="display:none;">Upload Image</a>
<?php
...
<?php
	...
	function _add_border_options( $group, $name = 'Borders', $show = false, $show_all = false ) {
		$form = $this->_form;
		
?>
	<?php $this->_start_show_hide_section( $name, $show ); ?>
		<?php if ( ! $show_all ) : ?>
			<tr><td colspan="2">Note: Since not all designs have borders in this location, modifying this option may not make any visible change in the design.</td></tr>
		<?php endif; ?>
...
<?php
	...
	function _add_title_options( $group, $name = 'Title', $description = 'These options style the title.', $advanced_options = false ) {
		...
		$this->_add_section_title( $name );
		$this->_add_section_description( $description );
	...
	function _add_basic_module_options( $group, $var, $name ) {
		...
		if ( 'content' == $var )
			$this->_add_section_description( "These options control the styling of the $name Module. The post and page content can be styled in the Post/Page Content Styling section. Comments can be styled in the Comments Styling section." );
		else
			$this->_add_section_description( "These options control the styling of the $name Module." );
		
		$this->_add_background_options( "{$group}_bg" );
		$this->_add_border_options( "{$group}_bg" );
		
		$form->add_hidden( "{$group}-selector", ".builder-module.builder-module-$var", true );
		$form->add_hidden( "{$group}_bg-selector", ".builder-module.builder-module-$var", true );
