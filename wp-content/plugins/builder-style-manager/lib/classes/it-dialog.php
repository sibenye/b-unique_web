...
Version History
	0.0.1 - 2012-07-26 - Chris Jean
		Initial development version.
	1.0.0 - 2013-01-09 - Chris Jean
...
if ( ! class_exists( 'ITDialog' ) ) {
	class ITDialog {
		function get_link( $link, $options = '' ) {
		...
		function add_enqueues() {
			...
			$vars = array(
				'no_iframes_message' => __( 'This feature requires inline frames. You have iframes disabled or your browser does not support them.', 'it-l10n-builder-style-manager' ),
				'loading_image'      => ITDialog::get_loading_image_url(),
				'close_message'      => __( 'Close', 'it-l10n-builder-style-manager' ),
		...
		function get_loading_image_url() {
			$plugin_url = ITUtility::get_url_from_file( dirname( __FILE__ ) );
			
			return "$plugin_url/images/it-dialog-loading.gif";
		}
		
		function render( $content_callback, $options = array() ) {
			global $current_user;
...
<?php
		...
		function add_form_submission_message( $message = '' ) {
