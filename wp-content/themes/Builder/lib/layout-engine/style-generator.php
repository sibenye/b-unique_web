...
Version History
	1.4.0 - 2012-10-22 - Chris Jean
		Added generator versioning to force stylesheet regeneration when the core generation code updates.
		Added "print render comments" when $_REQUEST['builder_debug'] is not empty.
	1.4.1 - 2012-10-23 - Chris Jean
		Bumped generator_version to 3.
		Added fix to prevent from trying to load a stylesheet if Builder isn't rendering the view (such as a plugin
			taking over the rendering process. This is done by checking to see if $this->layout is empty.
	1.4.2 - 2012-10-25 - Chris Jean
		Bumped generator_version to 4.
	1.4.3 - 2012-12-06 - Chris Jean
		Added a proper error handler for when a generated stylesheet file cannot be saved.
		...
		if ( builder_theme_supports( 'builder-disable-stylesheet-cache' ) ) {
			...
			if ( ! empty( $this->layout['stylesheet_file'] ) ) {
				if ( is_string( $this->layout['stylesheet_file'] ) && file_exists( $this->layout['stylesheet_file'] ) ) {
					...
					echo "\n<!--\n\t" . __( 'Builder Error: Unable to find a writable location to store the generated stylesheet. Check directory permissions in the wp-content/uploads directory.', 'it-l10n-Builder' ) . "\n";
					echo "\t" . sprintf( __( 'Error Message (%s): %s', 'it-l10n-Builder' ), $this->layout['stylesheet_file']->get_error_code(), $this->layout['stylesheet_file']->get_error_message() ) . "\n";
					echo "-->\n";
				}
			}
