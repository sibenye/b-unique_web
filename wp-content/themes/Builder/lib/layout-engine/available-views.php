...
function builder_set_available_views( $views ) {
	$new_views = array(
		...
		'is_search'				=> array(
			'name'			=> _x( 'Search Results', 'view', 'it-l10n-Builder' ),
			'priority'		=> '5',
			'description'	=> __( 'A search results page', 'it-l10n-Builder' ),
		),
		'is_404'				=> array(
...
function builder_add_custom_taxonomies( $views ) {
	...
	foreach ( (array) $custom_taxonomies as $taxonomy => $name ) {
		$new_views["builder_is_custom_taxonomy|$taxonomy"] = array(
			...
			'priority'    => '15',
			'description' => sprintf( _x( 'Any %s taxonomy entry', 'view description', 'it-l10n-Builder' ), $name ),
		);
	}
