...
Version History
	0.0.1 - 2011-08-31 - Chris Jean
		Built from version 3.4.1 of layout-engine.php.
	1.0.0 - 2011-10-06 - Chris Jean
		Release ready.
	1.1.0 - 2011-10-19 - Chris Jean
		Added action builder_force_layout_selection.
		Removed references to TEMPLATEPATH and STYLESHEETPATH.
	1.1.1 - 2011-12-12 - Chris Jean
		Switched print_render_comments() to the builder_layout_engine_render_header action.
		Forced call to setup_layout() if render() is called and _layout_id hasn't been set.
...
if ( ! class_exists( 'BuilderLayoutSelector' ) ) {
	class BuilderLayoutSelector {
		...
		function BuilderLayoutSelector() {
		...
		function render( $view ) {
			...
			if ( ! builder_theme_supports( 'builder-3.0' ) && ! file_exists( "$stylesheet_path/$view" ) && file_exists( "$template_path/legacy-templates/$view" ) )
				require_once( "$template_path/legacy-templates/$view" );
		...
		function filter_body_classes( $classes ) {
			$classes[] = "builder-template-" . substr( $this->_view, 0, -4 );
		...
		function setup_layout() {
			...
			else if ( isset( $post ) && ! empty( $post->post_type ) ) {
