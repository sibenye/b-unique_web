...
?>
	<p><label><?php $form->add_radio( 'method', 'add' ); ?> <?php _e( '<strong>Add Layouts:</strong> Add the Layouts from this export to this site\'s existing Layouts. The Views in this export will not be imported to this site.', 'it-l10n-Builder' ); ?></label></p>
	<p><label><?php $form->add_radio( 'method', 'custom' ); ?> <?php printf( __( '<strong>Custom:</strong> Selectively choose how to import the %s.', 'it-l10n-Builder' ), $name ); ?></label></p>
	<p><label><?php $form->add_radio( 'method', 'replace' ); ?> <?php printf( __( '<strong>Replace:</strong> Delete the %1$s from this site and replace with the %1$s in the export file.', 'it-l10n-Builder' ), $name ); ?></label></p>
	<p><label><?php $form->add_radio( 'method', 'skip' ); ?> <?php printf( __( '<strong>Skip:</strong> Do not import %1$s from the export file. The site\'s %1$s will remain unchanged.', 'it-l10n-Builder' ), $name ); ?></label></p>
<?php
		...
		function show_import_customizations_form( $form, $info, $data, $post_data ) {
			...
			echo '<h4>' . __( 'Select Layouts from the Import File to Add to Site', 'it-l10n-Builder' ) . "</h4>\n";
			echo "<div style='margin-left:20px;'>\n";
			echo '<p>' . __( 'Checking a layout below will add that layout from the import file to the site.', 'it-l10n-Builder' ) . "</p>\n";
...
<?php
			...
			echo '<h4>' . __( 'Select Layouts from the Site to Keep', 'it-l10n-Builder' ) . "</h4>\n";
			echo "<div style='margin-left:20px;'>\n";
			echo '<p>' . __( 'Unchecking one of the following layouts will remove it from the site.', 'it-l10n-Builder' ) . "</p>\n";
...
<?php
			...
			echo '<h4>' . __( 'Select Views from the Import File to Add to Site', 'it-l10n-Builder' ) . "</h4>\n";
			echo "<div style='margin-left:20px;'>\n";
			echo '<p>' . __( 'Checking a view below will add that view from the import file to the site.', 'it-l10n-Builder' ) . "</p>\n";
...
?>
	...
	<?php elseif ( false === $description['failed'] ) : ?>
		<?php $description = sprintf( __( '<strong>%1$s</strong> uses the export file\'s &quot;%2$s&quot; layout', 'it-l10n-Builder' ), $description['description'], $description['layout'] ); ?>
		<p><label><?php $form->add_check_box( $id ); ?> <?php echo $description; ?></label></p>
	<?php else : ?>
		<p><?php $form->add_check_box( $id, array( 'disabled' => 'disabled' ) ); ?> <?php printf( __( '<span style="color:red;">%s</span> &mdash; This view does not exist on the site.', 'it-l10n-Builder' ), $description['description'] ); ?></p>
	<?php endif; ?>
<?php
			...
			echo '<h4>' . __( 'Select Views from the Site to Keep', 'it-l10n-Builder' ) . "</h4>\n";
			echo "<div style='margin-left:20px;'>\n";
			echo '<p>' . __( 'Unchecking one of the following views will remove it from the site.', 'it-l10n-Builder' ) . "</p>\n";
...
?>
	<?php if ( ! is_wp_error( $description ) ) : ?>
		<?php $description = sprintf( __( '<strong>%1$s</strong> uses the site\'s &quot;%2$s&quot; layout', 'it-l10n-Builder' ), $description['description'], $description['layout'] ); ?>
		<?php $form->set_option( $id, 1 ); ?>
		<p><label><?php $form->add_check_box( $id ); ?> <?php echo $description; ?></label></p>
	<?php else : ?>
		<p><?php $form->add_check_box( $id, array( 'disabled' => 'disabled' ) ); ?> <?php _e( 'This View is broken and cannot be imported.', 'it-l10n-Builder' ); ?></p>
	<?php endif; ?>
<?php
...
//			ITUtility::print_js_script( 'jQuery(\'.builder_tooltip\').tooltip({track: false,delay: 0,showURL: false,showBody: " - ",fade: 250});' );
		...
		function _get_view_description( $id, $data, $meta = null ) {
			...
			if ( false === strpos( $id, '__' ) )
				return new WP_Error( 'builder-data-source-layout-views-get-view-description-unknown', __( 'Invalid view', 'it-l10n-Builder' ) );
...
/*			if ( 'replace' === $post_data['method'] )
			...
			$setting_descriptions = array(
				'description'	=> __( 'Layout Name', 'it-l10n-Builder' ),
				'width'			=> __( 'Layout Width', 'it-l10n-Builder' ),
				'custom_width'	=> __( 'Layout Width', 'it-l10n-Builder' ),
				'extension'		=> __( 'Extension', 'it-l10n-Builder' ),
				'disable_style'	=> __( 'Extension Style', 'it-l10n-Builder' ),
				'modules'		=> __( 'Modules', 'it-l10n-Builder' ),
			);
			...
			foreach ( (array) $active_layouts['import'] as $guid ) {
				...
				echo '<h4>' . sprintf( __( 'Resolve Layout &quot;%s&quot; Conflicts', 'it-l10n-Builder' ), $import_data['description'] ) . "</h4>\n";
				...
				if ( isset( $layout_conflicts[$guid]['guid'] ) ) {
					...
					if ( empty( $diff ) ) {
						echo "<p>" . sprintf( __( 'This Layout is a duplicate of the &quot;%s&quot; Layout that is currently on the site. All the settings are identical including the Layout\'s ID. Since each Layout must have a unique ID, please select an option to resolve this conflict.', 'it-l10n-Builder' ), $layout_settings['layouts'][$guid]['description'] ) . "</p>\n";
					}
					else {
...
?>
	<div style="margin:20px;">
		<p><label><?php $form->add_radio( 'actions[]', 'new_guid' ); ?> <?php _e( '<strong>New ID:</strong> Create a new ID for the Layout being imported.', 'it-l10n-Builder' ); ?></label></p>
		<p><label><?php $form->add_radio( 'actions[]', 'replace' ); ?> <?php printf( __( '<strong>Replace:</strong> Remove this site\'s &quot;%s&quot; Layout and replace it with this one for the import.', 'it-l10n-Builder' ), $layout_settings['layouts'][$guid]['description'] ); ?></label></p>
		<p><label><?php $form->add_radio( 'actions[]', 'skip' ); ?> <?php _e( '<strong>Skip:</strong> Do not import this Layout.', 'it-l10n-Builder' ); ?></label></p>
	</div>
<?php
				...
				if ( isset( $layout_conflicts[$guid]['description'] ) ) {
					echo '<p>' . __( 'A Layout on the site already has a Layout with this name. Please choose a new name for this Layout.', 'it-l10n-Builder' ) . "</p>\n";
...
?>
	<div style="margin:20px;">
		<p><label><?php __( 'New Name:', 'it-l10n-Builder' ); ?> <?php $form->add_text_box( 'new_description' ); ?></label></p>
...
<?php
			...
			foreach ( (array) $active_views['import'] as $id ) {
				...
				echo '<h4>' . sprintf( __( 'Resolve View &quot;%s&quot; Conflicts', 'it-l10n-Builder' ), $view_name['description'] ) . "</h4>\n";
...
//				$import_data['description'] = 'Blah';
//				$import_data['width'] = '780';
//				$import_data['extension'] = 'blah';
				
				if ( isset( $view_conflicts[$id]['id'] ) ) {
					echo "<p>" . sprintf( __( 'Both the site and the export data have a layout set for this View. The site uses the site\'s &quot;%1$s&quot; layout for this View. The export data uses the export\'s &quot;%2$s&quot; layout for this View. Please use the drop-down below to select what layout this view should use.', 'it-l10n-Builder' ), $layout_settings['layouts'][$layout_settings['views'][$id]['layout']]['description'], $data['layouts'][$import_data]['description'] ) . "</p>\n";
...
?>
	<div style="margin:20px;">
		<p><label><?php _e( 'Select a Layout for this View:', 'it-l10n-Builder' ); ?> <?php $form->add_drop_down( 'set_layout', $layout_options ); ?></label></p>
	</div>
<?php
			...
			if ( 'custom' === $post_data['method'] ) {
				...
				echo '<h4>' . __( 'Select the Default Layout', 'it-l10n-Builder' ) . "</h4>\n";
				
				echo '<p>' . _e( 'The Default Layout is used for all views that do not have a specific layout set. You can use the drop-down below to select which Layout you want to use as the Default Layout.', 'it-l10n-Builder' ) . "</p>\n";
				echo '<p>' . sprintf( __( 'Note: The current site Default Layout is the site\'s &quot;%1$s&quot; Layout and the export data Default Layout is the export data\'s &quot;%2$s&quot; Layout.', 'it-l10n-Builder' ), $current_default, $import_default ) . "</p>\n";
...
?>
<div style="margin:20px;">
	<p><label><?php _e( 'Select a Default Layout:', 'it-l10n-Builder' ); ?> <?php $form->add_drop_down( 'default_layout', $layout_options ); ?></label></p>
</div>
<?php
...
/*			foreach ( (array) $data as $guid => $layout_data ) {
		...
		function show_import_process( $form, $info, $data, $post_data ) {
			...
			if ( ! empty( $final_layout_settings['actions']['kept'] ) ) {
				echo '<h4>' . __( 'Keeping Site Layouts', 'it-l10n-Builder' ) . "</h4>\n";
				echo "<div style='margin-left:20px;'>\n";
				echo "<p>" . __( 'The following Layouts on the site will be kept:', 'it-l10n-Builder' ) . "</p>\n";
				echo "<ul>\n";
			...
			if ( ! empty( $final_layout_settings['actions']['imported'] ) ) {
				echo '<h4>' . __( 'Importing Layouts', 'it-l10n-Builder' ) . "</h4>\n";
				echo "<div style='margin-left:20px;'>\n";
				echo "<p>" . __( 'The following Layouts from the export file will be imported:', 'it-l10n-Builder' ) . "</p>\n";
				echo "<ul>\n";
				...
				foreach ( (array) $final_layout_settings['actions']['imported'] as $guid ) {
					...
					if ( isset( $final_layout_settings['actions']['renamed'][$guid] ) )
						$renamed = '&nbsp;&nbsp;&nbsp;<i>' . sprintf( __( '(renamed from &quot;%s&quot;)', 'it-l10n-Builder' ), $final_layout_settings['actions']['renamed'][$guid] ) . '</i>';
			...
			if ( ! empty( $final_layout_settings['actions']['removed'] ) ) {
				echo '<h4>' . __( 'Removing Site Layouts', 'it-l10n-Builder' ) . "</h4>\n";
				echo "<div style='margin-left:20px;'>\n";
				echo "<p>" . __( 'The following Layouts on the site will be removed:', 'it-l10n-Builder' ) . "</p>\n";
				echo "<ul>\n";
...
/*			if ( ! empty( $final_layout_settings['actions']['skipped'] ) ) {
				echo '<h4>'. __( 'Skipping Imported Layouts', 'it-l10n-Builder' ) . "</h4>\n";
				echo "<div style='margin-left:20px;'>\n";
				echo "<p>" . __( 'The following Layouts from the export file will not be imported:', 'it-l10n-Builder' ) . "</p>\n";
				echo "<ul>\n";
			...
			if ( ( 'add' !== $post_data['method'] ) && ! empty( $final_view_settings['data'] ) ) {
				echo '<h4>' . __( 'Final View Settings', 'it-l10n-Builder' ) . "</h4>\n";
				echo "<div style='margin-left:20px;'>\n";
				echo "<p>" . __( 'The site will use the following Views:', 'it-l10n-Builder' ) . "</p>\n";
				echo "<ul>\n";
				...
				foreach ( (array) $final_view_settings['data'] as $id => $guid ) {
					...
					$description = sprintf( __( '<strong>%1$s</strong> uses the &quot;%2$s&quot; layout', 'it-l10n-Builder' ), $description['description'], $description['layout'] );
			...
			if ( 'add' !== $post_data['method'] ) {
				echo '<h4>' . __( 'Default Layout', 'it-l10n-Builder' ) . "</h4>\n";
				echo "<div style='margin-left:20px;'>\n";
...
?>
	<div title="title - tooltip test" class="builder_tooltip"><strong><?php _e( 'Name:', 'it-l10n-Builder' ); ?></strong> <?php echo $data['description']; ?></div>
	<div><strong><?php _e( 'Width:', 'it-l10n-Builder' ); ?></strong> <?php echo $data['width']; ?> pixels</div>
	<div><strong><?php _e( 'Modules:', 'it-l10n-Builder' ); ?></strong></div>
	<ul>
