...
Version History
	2.4.0 - 2011-12-20 - Chris Jean
		Updated the system to use a more straight-forward way of keeping track of sidebars
		Changed sidebar naming to add full_name and short_name
	2.4.1 - 2011-12-20 - Chris Jean
		Fixed problem caused by WordPress 3.3's orphaned widgets code. Widgets now properly remain with Builder and
			Builder child themes when switching themes (even to non-Builder themes).
...
class BuilderSidebars {
	...
	function BuilderSidebars() {
		...
		add_action( 'after_switch_theme', array( $this, 'admin_register_all_sidebars' ), 0 );
	...
	function dynamic_sidebar( $index, $name = null ) {
		...
		if ( empty( $sidebar_index ) ) {
			...
			$instance = array(
				...
				'text'   => sprintf( __( 'This section is widgetized. If you would like to add content to this section, you may do so by using the Widgets panel from within your WordPress Admin Dashboard. This Widget Section is called "<strong>%s</strong>"', 'it-l10n-Builder' ), $name ),
				'filter' => 'on',
			);
