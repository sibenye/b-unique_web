...
Version History
	...
	2.3.1 - 2011-08-23 - Chris Jean
		Changed "Layouts and Views" to "Layouts & Views" due to new WP
			Dashboard styling
		Fixed links to Layouts in the Views listing
	2.3.2 - 2011-10-06 - Chris Jean
		Added a more-robust directory check to avoid problems with directory
			restrictions
	2.3.3 - 2011-12-09 - Chris Jean
...
if ( ! class_exists( 'BuilderLayoutEditor' ) ) {
	class BuilderLayoutEditor extends ITCoreClass {
		...
		function BuilderLayoutEditor() {
			...
			$this->_menu_title = _x( 'Layouts & Views', 'menu title', 'it-l10n-Builder' );
...
<?php
		...
		function _modify_module_settings( $data = false ) {
			...
			if ( method_exists( $this->_modules[$module], 'edit' ) ) {
				...
				echo '<h1>' . sprintf( __( 'Modify %s Module Settings', 'it-l10n-Builder' ), $this->_modules[$module]->_name ) . "</h1>";
...
<?php
		...
		function _delete_layout_screen() {
			...
			if ( count( $this->_options['layouts'] ) < 2 ) {
				...
				echo '<p class="submit">';
				$form->add_submit( 'cancel', array( 'value' => __( 'Cancel', 'it-l10n-Builder' ), 'class' => 'button-secondary', 'onclick' => 'it_dialog_remove();' ) );
				echo '</p>';
...
?>
	<?php $form->start_form(); ?>
		...
		<p class="submit">
			...
			<?php $form->add_submit( 'cancel', array( 'value' => __( 'Cancel', 'it-l10n-Builder' ), 'class' => 'button-secondary', 'onclick' => 'it_dialog_remove();' ) ); ?>
		</p>
...
<?php
		...
		function _setDefaultLayout() {
			...
			$set_default_link = "<div class='row-actions'><a href='" . ITDialog::get_link( "{$this->_self_link}&set_default_layout_screen=$original_default" ) . "' class='it-dialog' title='" . __( 'Set this layout as the default', 'it-l10n-Builder' ) . "'>";
...
?>
	<?php $form->start_form(); ?>
		<table style="border-spacing:0px 10px;">
			<h1><?php _e( 'Select a module to add to the layout', 'it-l10n-Builder' ); ?></h1>
	...
	<script type="text/javascript">
		...
		jQuery( function() {
			it_dialog_add_form_submission_message( "<?php echo esc_js( __( 'Creating module.', 'it-l10n-Builder' ) ); ?>" );
		} );
	</script>
<?php
						...
						<tr id="entry-<?php echo $id; ?>"<?php echo $class; ?>>
							...
							<td>
								...
								<div class="row-actions">
									...
									<span class="duplicate"><a href="<?php echo ITDialog::get_link( "{$this->_self_link}&duplicate_layout_screen=$id", array( 'width' => '250' ) ); ?>" class="it-dialog" title="<?php _e( 'Duplicate Layout', 'it-l10n-Builder' ); ?>"><?php _e( 'Duplicate', 'it-l10n-Builder' ); ?></a> | </span>
									<span class="delete"><a href="<?php echo ITDialog::get_link( "{$this->_self_link}&delete_layout_screen=$id" ); ?>" class="it-dialog" title="<?php _e( 'Delete Layout', 'it-l10n-Builder' ); ?>"><?php _e( 'Delete', 'it-l10n-Builder' ); ?></a></span>
								</div>
							</td>
							<td class="set_default" title="<?php _e( 'The default layout is used for all views that don\'t have a specific view set.', 'it-l10n-Builder' ); ?>">
								...
								<?php else : ?>
									<div class="row-actions">
										<a href="<?php echo ITDialog::get_link( "{$this->_self_link}&set_default_layout_screen=$id" ); ?>" class="it-dialog" title="<?php _e( 'Set this layout as the default', 'it-l10n-Builder' ); ?>"><?php _e( 'Set as default', 'it-l10n-Builder' ); ?></a>
									</div>
								<?php endif; ?>
							</td>
...
?>
	<?php $form->start_form(); ?>
		<h1><?php echo ( empty( $_REQUEST['original_view'] ) ) ? __( 'Add View', 'it-l10n-Builder' ) : __( 'Modify View', 'it-l10n-Builder' ); ?></h1>
	...
	<div class="wrap">
		<?php $form->start_form(); ?>
			...
			<div id="no-views-container">
				...
				<a href="<?php echo $add_link; ?>" class="it-dialog button-secondary link-secondary"><?php _e( 'Add View', 'it-l10n-Builder' ); ?></a>
			</div>
			...
			<div id="views-container">
					...
					<tbody>
						...
						<?php foreach ( (array) $views as $view_id => $view ) : ?>
							<tr class="view-entry" id="view-<?php echo $this->_parse_view_id( $view_id ); ?>"<?php echo $class; ?>>
								<td>
									<strong><a class="view-name it-dialog" href="<?php echo ITDialog::get_link( "{$this->_self_link}&modify_view_screen=1&original_view=$view_id", "modal=true&max-width=400" ); ?>" title="<?php _e( 'Modify View', 'it-l10n-Builder' ); ?>"><?php echo $view['name']; ?></a></strong><br />
									<div class="row-actions">
										<span class="edit"><a href="<?php echo ITDialog::get_link( "{$this->_self_link}&modify_view_screen=1&original_view=$view_id", "modal=true&max-width=400" ); ?>" class="it-dialog" title="Modify View"><?php _e( 'Edit', 'it-l10n-Builder' ); ?></a> | </span>
										<span class="delete"><a href="<?php echo ITDialog::get_link( "{$this->_self_link}&delete_view_screen=$view_id" ); ?>" class="it-dialog" title="Remove View Customization"><?php _e( 'Remove', 'it-l10n-Builder' ); ?></a></span>
									</div>
								</td>
								<td><?php echo $view['description']; ?></td>
								<td class="view-layout">
									<?php if ( isset( $this->_options['layouts'][$view['layout']] ) ) : ?>
										<a href="<?php echo $this->_tabless_self_link; ?>&editor_tab=layouts&layout=<?php echo $view['layout']; ?>" title="<?php _e( 'Modify Layout', 'it-l10n-Builder' ); ?>">
				...
				<div class="tablenav">
					<div class="alignleft actions">
						<a href="<?php echo $add_link; ?>" class="button-secondary link-secondary it-dialog"><?php _e( 'Add View', 'it-l10n-Builder' ); ?></a>
					</div>
			...
			<div id="new-view-container" style="display:none;">
				<table>
					<tr class="view-entry" id="view-%parsed_view_id%" style="display:none;">
						<td>
							<strong><a class="view-name it-dialog" href="<?php echo ITDialog::get_link( "{$this->_self_link}&modify_view_screen=1&original_view=%view_id%", "modal=true" ); ?>" title="<?php _e( 'Modify View', 'it-l10n-Builder' ); ?>">%view_name%</a></strong><br />
							<div class="row-actions">
								<span class="edit"><a href="<?php echo ITDialog::get_link( "{$this->_self_link}&modify_view_screen=1&original_view=%view_id%", "modal=true" ); ?>" class="it-dialog" title="Modify View"><?php _e( 'Edit', 'it-l10n-Builder' ); ?></a> | </span>
								<span class="delete"><a href="<?php echo ITDialog::get_link( "{$this->_self_link}&delete_view_screen=%view_id%" ); ?>" class="it-dialog" title="Remove View Customization"><?php _e( 'Remove', 'it-l10n-Builder' ); ?></a></span>
							</div>
						</td>
