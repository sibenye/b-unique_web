...
Version History
	1.2.0 - 2011-10-06 - Chris Jean
		Changed init hook to admin_init
	1.3.0 - 2011-10-19 - Chris Jean
		Changed camelcase functions and vars with underscore naming
		Updated the help text handler to be compatible with the WP 3.3 release
	1.3.1 - 2012-09-26 - Chris Jean
		Fixed URL generation for menu icon.
...
<?php
	...
	function add_pages() {
		...
		$tutorial_menu_name = __( 'Start Here', 'it-l10n-Builder' );
		...
		if ( ! empty( $wp_theme_page_name ) ) {
			...
			$this->_page_ref = add_theme_page( sprintf( __( '%s Start Here', 'it-l10n-Builder' ), $wp_theme_name ), sprintf( __( '%s Start Here', 'it-l10n-Builder' ), $wp_theme_name ), $menu_capability, $this->_page, array( $this, 'index' ) );
		}
