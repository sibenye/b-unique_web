...
Version History
	...
	1.1.1 - 2011-10-06 - Chris Jean
		Fixed bug that prevents the Analytics settings from collapsing
	1.1.2 - 2011-10-19 - Chris Jean
		Fixed typo in link to Layouts Editor
	1.2.0 - 2011-12-05 - Chris Jean
		Added options for GoSquared.com integration
	1.3.0 - 2011-12-09 - Chris Jean
		Added settings for the [gallery] shortcode
	1.3.1 - 2011-12-09 - Chris Jean
		Fixed bad link for the GoSquared analytics video
	1.3.2 - 2011-12-12 - Chris Jean
		Improved the settings text for the gallery shortcode options
	1.3.3 - 2012-04-17 - Chris Jean
		Set a default array value for $comment_post_types to prevent warnings in
			the event that no post types supporting comments are found.
...
?>
	<p><?php printf( __( '<a href="%1$s">Web analytics software</a> tracks what content visitors to your site are interested in and how they get to your site. Builder offers an easy way to integrate analytics tracking for <a href="%2$s">Google Analytics</a>, <a href="%3$s">Woopra</a>, and <a href="%4$s">GoSquared</a>. Use the settings below to easily add the desired tracking code to your site.', 'it-l10n-Builder' ), 'http://en.wikipedia.org/wiki/Web_analytics', 'http://www.google.com/analytics/', 'http://www.woopra.com/', 'https://www.gosquared.com/join/ithemes?ref=11928' ); ?></p>
	
	<p><?php _e( 'Before activating Builder\'s built-in support for any of these services, first disable any plugins offering the same feature. Failure to do so can result in each visitor being counted multiple times, which will badly skew your data.', 'it-l10n-Builder' ); ?></p>
	...
	<div class="google_analytics_enable-option">
		<p><?php printf( __( 'Your site is uniquely identified in Google Analytics by an Account ID. An example Account ID is UA-12345-6. For help on finding your Account ID, watch <a href="%s" class="it-dialog">this video</a>.', 'it-l10n-Builder' ), ITDialog::get_link( "{$this->_parent->_self_link}&show_video=LhZ-Zwy06Ik&video_width=848&video_height=504" ) ); ?></p>
		<p><label><?php _e( 'Google Analytics Account ID', 'it-l10n-Builder' ); ?> <?php $form->add_text_box( 'google_analytics_account_id' ); ?> <?php _e( '(required)', 'it-l10n-Builder' ); ?></label></p>
	...
	<div class="woopra_enable-option">
		<p><?php printf( __( 'By default (when the Woopra Domain input below is empty), Woopra will use the domain of the "Site address (URL)" configured in <a href="%s">Settings &gt; General</a>. This can be changed by supplying a new domain below.', 'it-l10n-Builder' ), admin_url( 'options-general.php' ) ); ?></p>
		<p><label><?php _e( 'Woopra Domain', 'it-l10n-Builder' ); ?> <?php $form->add_text_box( 'woopra_domain' ); ?> <?php _e( '(optional)', 'it-l10n-Builder' ); ?></label></p>
		
		<br />
	</div>
	
	
	<p><label><?php $form->add_check_box( 'gosquared_enable', array( 'class' => 'show-hide-toggle' ) ); ?> <?php _e( 'Enable GoSquared', 'it-l10n-Builder' ); ?></label></p>
	
	<div class="gosquared_enable-option">
		<p><?php printf( __( 'GoSquared identifies your site using a Site Token. The Site Token for your site can be found on the Tracking Code page in your <a href="%1$s">GoSquared Settings</a>. You can find more information about the Site Token and how to find it on the <a href="%2$s">GoSquared FAQ</a>.', 'it-l10n-Builder' ), 'https://www.gosquared.com/settings/', 'http://www.gosquared.com/support/wiki/faqs#faq-site-token' ); ?></p>
		<p><label><?php _e( 'Site Token', 'it-l10n-Builder' ); ?> <?php $form->add_text_box( 'gosquared_site_token' ); ?> <?php _e( '(required)', 'it-l10n-Builder' ); ?></label></p>
	</div>
...
?>
	<p><?php printf( __( 'Builder has a couple of different rendering methods for <a href="%s">[gallery] shortcodes</a>:', 'it-l10n-Builder' ), 'http://codex.wordpress.org/Gallery_Shortcode' ); ?></p>
	
	<dl class="indented-definition-list">
		<dt><?php _e( 'Columns by Default', 'it-l10n-Builder' ); ?></dt>
		<dd><?php _e( 'Most themes render gallery items in three columns by default. This means that if your shortcode text is just <code>[gallery]</code>, then the gallery items will be in three columns. The images will become smaller or larger to fit the space. If the images are too small, simply change the shortcode to use fewer columns.', 'it-l10n-Builder' ); ?></dd>
		<dd><?php _e( 'Using this setting, galleries can still be rendered as a free-flowing list (described below) by setting the columns attribute of the shortcode to "auto". For example, <code>[gallery columns="auto"]</code>.', 'it-l10n-Builder' ); ?></dd>
		
		<dt><?php _e( 'Free-flowing List by Default', 'it-l10n-Builder' ); ?></dt>
		<dd><?php _e( 'In older versions of Builder, the columns value of the shortcode would be ignored. Instead, each gallery item would be rendered at the designated size (the thumbnail size by default) and the items would simply flow to fill the space. For example, a wide layout may result in four columns while a more narrow layout would result in three columns. This option can be used to restore the old behavior.', 'it-l10n-Builder' ); ?></dd>
		<dd><?php _e( 'Even with this option specified, current versions of Builder do respect the column count specified in the shortcode text. For example, changing the shortcode to <code>[gallery columns="5"]</code> results in a five column gallery. Since selecting three columns from the gallery shortcode editor results in a shortcode of <code>[gallery]</code>, a three-column gallery would require manual modification of the shortcode to change it to <code>[gallery columns="3"]</code>.', 'it-l10n-Builder' ); ?></dd>
	</dl>
	
	<p><?php _e( 'Use the settings below to customize how Builder handles [gallery] shortcodes.', 'it-l10n-Builder' ); ?></p>
	
	<hr />
	
	<p><?php _e( 'How do you want galleries to be rendered?', 'it-l10n-Builder' ); ?></p>
	<ul class="no-bullets">
		<li><label for="gallery_default_render_method-columns"><?php $form->add_radio( 'gallery_default_render_method', array( 'value' => 'columns' ) ); ?> <?php _e( 'Columns by Default (default)', 'it-l10n-Builder' ); ?></label></li>
		<li><label for="gallery_default_render_method-auto"><?php $form->add_radio( 'gallery_default_render_method', array( 'value' => 'auto' ) ); ?> <?php _e( 'Free-flowing List by Default', 'it-l10n-Builder' ); ?></label></li>
	</ul>
<?php
...
?>
	<p><?php _e( 'Builder offers a large number of built-in features. These features can be enabled and disabled using the following options.', 'it-l10n-Builder' ); ?></p>
	<hr />
<?php
...
?>
	...
	<?php else : ?>
		<p><?php _e( 'It appears that comments are disabled on your site due to a plugin or other configuration. All post types are reporting that they do not support commenting.', 'it-l10n-Builder' ); ?></p>
	<?php endif; ?>
<?php
