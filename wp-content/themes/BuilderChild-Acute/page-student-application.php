...
$post_param_names = array('admission_session', 'first_name', 'last_name', 'email', 'marital_status', 'dob', 'gender', 'home_phone', 'mobile_phone', 'other_phone', 'education', 'address', 'city', 'state', 'zipcode', 'country', 'ssn', 'dl_number', 'dl_issue', 'applying_for', 'student_type', 'period', 'committment', 'employer_name', 'employer_phone', 'employer_address', 'employer_city', 'employer_state', 'employer_zipcode', 'employer_country', 'emergency_name', 'emergency_home_number', 'emergency_mobile_number', 'emergency_relationship', 'emergency_address', 'emergency_city', 'emergency_state', 'emergency_zipcode', 'emergency_country', 'physical_disability', 'medication', 'summary', 'referrer');
			...
			$content = array("student_application" => $value_pair);
			$headers = array('Content-Type: application/json');
			//$content = '{"student_application":{'.$value_pair.'}}';
			$request = new WP_Http;
...
//load variables
$edus = array('A High School Graduate','Attending College','A GED Graduate','Attending High School','Did not complete High School');
...
$diplomas = array('Master Barber','Registered Barber');
...
<form class="" action="" method="post">
	...
	<label class="label">Admission Session <span class="red">*</span></label><br />
	<select class="<?php echo $presence_validation['admission_session'] == 'fail'? 'invalid-input' :''; ?>" name="st_app[admission_session]">
	<option value="<?php echo $st_app['admission_session']; ?>" name="st_app[admission_session]" OnClick=""><?php echo $st_app['admission_session'];?></option>
	<?php foreach($sessions as $session): ?>
	<option value="<?php echo $session; ?>" name="st_app[admission_session]" OnClick=""><?php echo $session;?></option>
						<?php endforeach; ?>
    </select></td>
  </tr>
  <tr>
  ...
  <td class="field">
    <label class="label">Date of birth<span class="red"> *</span> mm/dd/yyyy or yyyy/mm/dd</label><br />
  ...
  <td class="field">
	...
	<input class="<?php echo $presence_validation['zipcode'] == 'fail'? 'invalid-input' :''; ?>" type="text" name="st_app[zipcode]" value="<?php echo $st_app['zipcode']; ?>" size="5" maxlength="7">
  ...
  <td class="field">
	...
	<input class="<?php echo $presence_validation['ssn'] == 'fail'? 'invalid-input' :''; ?>" type="text" name="st_app[ssn]" value="<?php echo $st_app['ssn']; ?>" size="9" maxlength="11">
    
  </td>
  <td class="field">
    <label class="label">Drivers License Number</label><br />
	<input class="<?php echo $dl_number-style; ?>" type="text" name="st_app[dl_number]" value="<?php echo $st_app['dl_number']; ?>" size="15" maxlength="25">
  ...
  <tr>
  <td class="field">
    <label class="label">Applying for<span class="red"> *</span></label><br />
  ...
  <td class="field">
    <label class="label">Will attend<span class="red"> *</span></label><br />
	<select class="<?php echo $presence_validation['student_type'] == 'fail'? 'invalid-input' :''; ?>" name="st_app[student_type]">
	<option value="<?php echo $st_app['student_type']; ?>" name="st_app[student_type]" OnClick=""><?php echo $st_app['student_type'];?></option>
	<?php foreach($stypes as $stype): ?>
	<option value="<?php echo $stype; ?>" name="st_app[student_type]" OnClick=""><?php echo $stype;?></option>
						<?php endforeach; ?>
    </select>
    
  </td>
  <td class="field">
    <label class="label">During<span class="red"> *</span></label><br />
  ...
  <tr>
  <td class="field"  colspan="3">
  	<label class="label">What_is_your_level_of_committment_right_now?</label>
  	<br/>
	<select class="<?php echo $committment-style; ?>" name="st_app[committment]">
	<option value="<?php echo $st_app['committment']; ?>" name="st_app[committment]" OnClick=""><?php echo $st_app['committment'];?></option>
	<option value="I am ready to get started" name="st_app[committment]" OnClick="">I am ready to get started</option>
	<option value="Gathering Information and would like to know what kind of financial aid is available" name="st_app[committment]" OnClick="">Gathering Information and would like to know what kind of financial aid is available</option>
	<option value="Gathering Information only" name="st_app[committment]" OnClick="">Gathering Information only</option>					
    </select>
  ...
  <td class="field">
	...
	<input class="<?php echo $employer_zipcode-style; ?>" type="text" name="st_app[employer_zipcode]" value="<?php echo $st_app['employer_zipcode']; ?>" size="5" maxlength="7">
  ...
  <td class="field">
    <label class="label">ZipCode<span class="red"> *</span></label><br />
	<input class="<?php echo $presence_validation['emergency_zipcode'] == 'fail'? 'invalid-input' :''; ?>" type="text" name="st_app[emergency_zipcode]" value="<?php echo $st_app['emergency_zipcode']; ?>" size="5" maxlength="7">
